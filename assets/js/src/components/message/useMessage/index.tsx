...
import { devUseWarning } from 'antd/lib/_util/warning';
import { ConfigContext } from 'antd/lib/config-provider';
import type { ComponentStyleConfig } from 'antd/lib/config-provider/context';
import useCSSVarCls from 'antd/lib/config-provider/hooks/useCSSVarCls';
import type {
    ArgsProps,
    ConfigOptions,
    MessageInstance,
...
import useStyle from 'antd/lib/message/style';
import {useStyle as usePimcoreStyle} from '@Pimcore/components/message/message.style';
import {getMotion, wrapPromiseFn } from 'antd/lib/message/util';
...
const Wrapper: FC<PropsWithChildren<{ prefixCls: string }>> = ({ children, prefixCls }) => {
    ...
    const {styles: pimcoreStyles} = usePimcoreStyle();
    const styleClassNames = [
        classNames(hashId, cssVarCls, rootCls),
        pimcoreStyles.message
    ];
    return wrapCSSVar(
        <NotificationProvider classNames={{ list: styleClassNames.join(' ') }}>
            {children}
...
export default function index(messageConfig?: ConfigOptions) {
