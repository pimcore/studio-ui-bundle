...
import {
  api,
  type DocumentGetTreeApiArg
} from '@Pimcore/modules/document/document-api-slice-enhanced'
...
export const useNodeApiHook = (): NodeApiHookReturnType => {
  ...
  async function fetchRoot (id: number): Promise<DataTransformerReturnType | undefined> {
    ...
    return await Promise.resolve(createFallbackRootNode(id, 'document'))
  }
  ...
  async function fetch (node: DataTransformerSourceNode, args: DocumentGetTreeApiArg): Promise<DataTransformerReturnType | undefined> {
    const treeFetcher = dispatch(api.endpoints.documentGetTree.initiate(args, { forceRefetch: true }))
