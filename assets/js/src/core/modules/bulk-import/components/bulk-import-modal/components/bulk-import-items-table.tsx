...
import { useStyles } from './bulk-import-items-table.styles'

interface BulkImportItemsTableProps {
  items: BulkExportAvailableItem[]
  toggleItem: (type: string, name: string) => void
  isSelected: (type: string, name: string) => boolean
}

export const BulkImportItemsTable = ({ items, toggleItem, isSelected }: BulkImportItemsTableProps): React.JSX.Element => {
  const { styles } = useStyles()

  return (
    <div className='bulk-import-items-table'>
      {items.map((item) => (
        <Flex
          align='center'
