...
import {
  type ReportsColumnFilter,
  type ReportsFilterContext,
  type ReportsFilterContribution
} from '../reports-filter-types'
...
const buildColumnFilters = (
  ...
  getType: ReportsFilterContext['getType']
): ReportsColumnFilter[] => fieldFilters
...
const ReportsFieldFiltersControl = ({ value, onChange }: FilterControlProps<IDynamicFilter[]>): React.JSX.Element => {
  ...
  useEffect(() => {
    ...
    const newAddColumnMenu = filterableColumnConfigurationsList
      ?.filter((initialColumn) => !value.some((column) => initialColumn.name === column.name))
      ?.map((column) => ({
  ...
  }, [fullChartDetailData, reportDetailData, value])
  ...
  return (
    <>
      ...
      >
        <Flex vertical>
          { value.length === 0 && <Empty image={ Empty.PRESENTED_IMAGE_SIMPLE } /> }
          { value.length > 0 && (
...
export const reportsFieldFiltersDescriptor = defineFilter<IDynamicFilter[], ReportsFilterContribution, ReportsFilterContext>({
  key: 'fieldFilters',
  defaultValue: [],
  section: 'fields',
  order: 10,
