...
export const useNodeApiHook = (): NodeApiHookReturnType => {
  ...
  async function fetchApiHookResult (node: DataTransformerSourceNode, nodeState: NodeState): Promise<DataTransformerReturnType | undefined> {
    const treeFetcher = dispatch(api.endpoints.assetGetTree.initiate({ parentId: parseInt(node.id), pageSize: maxItemsPerNode!, page: nodeState.page, idSearchTerm: nodeState.searchTerm, ...treeFilterArgs }))
