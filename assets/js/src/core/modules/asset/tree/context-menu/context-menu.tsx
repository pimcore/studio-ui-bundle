...
export const AssetTreeContextMenu = (props: TreeContextMenuProps): React.JSX.Element => {
  ...
  const { renameTreeContextMenuItem } = useRename('asset', getContextMenuCacheKey('asset', 'rename', parseInt(props.node.id)))
  const { deleteTreeContextMenuItem } = useDelete('asset', getContextMenuCacheKey('asset', 'rename', parseInt(props.node.id)))
