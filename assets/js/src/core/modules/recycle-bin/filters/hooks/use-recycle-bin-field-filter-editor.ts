...
export const useRecycleBinFieldFilterEditor = (): UseRecycleBinFieldFilterEditorReturn => {
  ...
  const { values, setValue } = useRecycleBinDraftFilters()
  ...
  const initialFilters: FieldFiltersProps['data'] = useMemo(() => {
    ...
    for (const filter of fieldFilters) {
      const column = RECYCLE_BIN_FILTERABLE_FIELDS.find((col) => col.key === filter.key)
  ...
  const availableColumns = useMemo(() => RECYCLE_BIN_FILTERABLE_FIELDS.filter((column) => {
  ...
  const columnGroups = useMemo<Array<ColumnPickerGroup<RecycleBinFilterColumn>>>(() => {
    if (availableColumns.length === 0) {
      return []
    }

    return [{
      key: 'recycle-bin-columns',
      label: '',
