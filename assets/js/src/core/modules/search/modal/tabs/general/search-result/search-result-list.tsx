...
export interface SearchResultListProps {
  ...
  isLoading: boolean
  /** Shown instead of the empty state when the data source failed. */
  errorText?: string
  onPageChange: (page: number, pageSize: number) => void
  /** Bumps when the term/mode changes so hover selection resets with the new result set. */
  resetKey: string
}

/** List + hover detail + pagination for the All tab; the caller owns the data source and paging. */
export const SearchResultList = ({ data, isLoading, errorText, onPageChange, resetKey }: SearchResultListProps): React.JSX.Element => {
  ...
  }, [data, selectedItem, isLoading, errorText])
}
