...
import { api, type DataObjectGetGridApiArg, type DataObjectGetGridApiResponse } from '@Pimcore/modules/data-object/data-object-api-slice-enhanced'
...
jest.mock('@Pimcore/modules/element/listing/abstract/settings/use-settings', () => ({
  useSettings: () => ({
    useColumnMapper: () => ({
      shouldMapDataToColumn: (column: { key?: string | null }, columnToUpdate: { key?: string | null }) => column.key === columnToUpdate.key
    })
  })
}))

describe('useInlineEditApiUpdate (data object)', () => {
  it('returns an undo handle from updateCache that reverts the optimistic grid update', async () => {
    const getGridArgs: DataObjectGetGridApiArg = { classId: 'car', body: { folderId: 1 } }
    ...
    const gridData: DataObjectGetGridApiResponse = {
      totalItems: 1,
      items: [
        {
          id: 5,
          columns: [{ key: 'name', locale: null, value: 'old value' }]
        }
      ]
    }
    ...
    await store.dispatch(api.util.upsertQueryData('dataObjectGetGrid', getGridArgs, gridData))
    ...
    const getCachedValue = (): unknown => {
      const queryState = api.endpoints.dataObjectGetGrid.select(getGridArgs)(store.getState())
