...
/* import */ var _Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3 = __webpack_require__("./js/src/core/modules/user/roles/roles-api-slice.gen.ts");
/* import */ var _Pimcore_components_message_useMessage__rspack_import_4 = __webpack_require__("./js/src/core/components/message/useMessage/index.tsx");
/* provided dependency */ var $ReactRefreshRuntime$ = __webpack_require__("./node_modules/@rspack/plugin-react-refresh/client/reactRefresh.js");
var _s = $RefreshSig$();
/**
 * This source file is available under the terms of the
...
const toPerspectiveIds = (perspectives)=>perspectives.map((perspective)=>typeof perspective === 'string' ? perspective : perspective.id);
const useRoleHelper = ()=>{
    _s();
    const { t } = (0,react_i18next__rspack_import_2.useTranslation)();
    const messageApi = (0,_Pimcore_components_message_useMessage__rspack_import_4.useMessage)();
    const dispatch = (0,_sdk_app__rspack_import_0.useAppDispatch)();
    ...
    const handleNotification = async (successMessage, error)=>{
        if (error !== undefined) {
            var _error_data;
            await messageApi.error((error === null || error === void 0 ? void 0 : (_error_data = error.data) === null || _error_data === void 0 ? void 0 : _error_data.message) ?? t('error'));
        } else {
    ...
    async function fetchRoleById(props) {
        const { id } = props;
        const { data } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleGetById.initiate({
            id
        }));
        return data;
    }
    async function getRoleTree(props) {
        ...
        const { data } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleGetTree.initiate({
            parentId
        }));
        return data;
    }
    async function searchRoleByText(query) {
        const { data } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleSearch.initiate({
            searchQuery: query
        }));
        return data;
    }
    async function addNewRole(props) {
        ...
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleCreate.initiate({
            body: {
                parentId,
                name
            }
    ...
    async function addNewFolder(props) {
        const { parentId, name } = props;
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleFolderCreate.initiate({
            body: {
                parentId,
                name
            }
    ...
    async function removeRole(props) {
        const { id } = props;
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleDeleteById.initiate({
            id
        }));
    ...
    async function removeFolder(props) {
        const { id } = props;
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleFolderDeleteById.initiate({
            id
        }));
    ...
    async function cloneRole(props) {
        const { id, name } = props;
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleCloneById.initiate({
            id,
            body: {
                name
            }
    ...
    async function updateRoleById(props) {
        const { id, item } = props;
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleUpdateById.initiate({
            id,
            updateRole: {
                name: item.name,
    ...
    async function moveRoleById(props) {
        ...
        const { data, error } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleUpdateById.initiate({
            ...
            updateRole: {
                ...role,
                parentId,
                perspectives: toPerspectiveIds(role.perspectives)
            }
        }));
        await handleNotification(t('roles.save-item.success'), error);
        return data;
    ...
    async function getRoleCollection() {
        const { data } = await dispatch(_Pimcore_modules_user_roles_roles_api_slice_gen__rspack_import_3.api.endpoints.roleGetCollection.initiate());
        return data;
    }
    return {
        openRole,
...
_s(useRoleHelper, "QW/xDGLtEp/XicTmvVe+gEghQpI=", false, function() {
    return [
        react_i18next__rspack_import_2.useTranslation,
        _Pimcore_components_message_useMessage__rspack_import_4.useMessage,
        _sdk_app__rspack_import_0.useAppDispatch,
        _sdk_app__rspack_import_0.useAppSelector,
        _sdk_app__rspack_import_0.useAppSelector
    ];
