...
const helperTest = (mainVersion, compareVersion)=>{
    const fullFieldList = mainVersion.length > compareVersion.length ? mainVersion : compareVersion;
    console.log('---->>>>> fullFieldList: ', fullFieldList);
    return {
        mainVersion,
        compareVersion
    };
...
const versionsDataToTableData = (param)=>{
    ...
    for(let index = 0; index < mainVersionData.length; index++){
        ...
        const { mainVersion, compareVersion } = helperTest(data[0], data[1]);
...
//# sourceMappingURL=js_src_core_mf-bootstrap_ts.4cd6c46dc08d7430.hot-update.js.map