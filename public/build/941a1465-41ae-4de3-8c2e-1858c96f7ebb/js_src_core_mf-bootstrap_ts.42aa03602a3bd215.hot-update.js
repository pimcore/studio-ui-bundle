...
const versionsDataToTableData = (param)=>{
    ...
    const baseMap = new Map(mainVersion.map((item)=>[
            getFieldKey(item),
            item
        ]));
    const compareMap = new Map(compareVersion.map((item)=>[
            getFieldKey(item),
            item
        ]));
    const compareVersionData = data[1] ?? [];
    const fullVersionFieldList = mainVersionData.length >= compareVersionData.length ? mainVersionData : compareVersionData;
    const isComparisonMode = !(0,lodash__WEBPACK_IMPORTED_MODULE_0__.isEmpty)(compareVersionData);
    for(let index = 0; index < fullVersionFieldList.length; index++){
...
//# sourceMappingURL=js_src_core_mf-bootstrap_ts.42aa03602a3bd215.hot-update.js.map