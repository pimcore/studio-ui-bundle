...
const versionsDataToTableData = (param)=>{
    ...
    const baseMap = new Map(mainVersionData.map((item)=>[
            getFieldKey(item),
            item
        ]));
    const compareVersionData = data[1] ?? [];
    const compareMap = new Map(compareVersionData.map((item)=>[
            getFieldKey(item),
            item
        ]));
    const versionsMax = mainVersionData.length >= compareVersionData.length ? mainVersionData : compareVersionData;
    const isComparisonMode = !(0,lodash__WEBPACK_IMPORTED_MODULE_0__.isEmpty)(compareVersionData);
    for(let index = 0; index < versionsMax.length; index++){
        const mainVersionItem = baseMap.get();
...
//# sourceMappingURL=js_src_core_mf-bootstrap_ts.2c2816f794d56ec9.hot-update.js.map