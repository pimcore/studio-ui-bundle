...
## Details
New entries to the left sidebar can be added via the `ComponentRegistry` service from the DI container and the `leftSidebar.slot` [slot](../../01_Architecture_Overview/01_SDK_Overview/04_Component_Registry.md#slots). In this example, a heart icon is added, and clicking on it triggers a modal message.
